...
ADD start/*.* /opt/helloworld-jsp/

RUN chmod 755 /opt/helloworld-jsp/pkamo-helloworld-jsp-1.0-SNAPSHOT.war
RUN chmod 755 /opt/helloworld-jsp/tomcat-users.xml

#CMD exec /opt/helloworld-jsp/start.sh

ADD target/pkamo-helloworld-jsp-1.0-SNAPSHOT.war /usr/local/tomcat/webapps
